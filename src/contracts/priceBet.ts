...
export class PriceBet extends SmartContract {
  ...
  public unlock(msg: ByteString, sig: RabinSig, winnerSig: Sig) {
    ...
    assert(message.tradingPair == this.tradingPair, 'incorrect trading pair.');

    // Decide winner and check their signature.
    const winner =
      message.price >= this.targetPrice ? this.alicePubKey : this.bobPubKey;
    assert(this.checkSig(winnerSig, winner), 'Winner checkSig failed.');
  }

  static parsePubKey(response: { publicKey: ByteString }): RabinPubKey {
    return byteString2Int(response.publicKey + '00') as RabinPubKey;
  }

  static parseData(response: { data: ByteString }): ByteString {
